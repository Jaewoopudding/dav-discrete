...
def run(args, rank=None):
    assert args.batch_size_sgdd % args.training_batch_size == 0
    args.batch_size = args.batch_size_sgdd
    set_seed(args.seed)
    args_dict = vars(args)
    ...
    exp_name = f'SGDD_seed{args.seed}_batch_size_sgdd{args.batch_size_sgdd}_alpha{args.alpha_sgdd}_num_steps_sgdd{args.num_steps_sgdd}_division{args.division}_inner_epochs{args.inner_epochs}_training_batch_size{args.training_batch_size}_skip_steps{args.skip_steps}_epc{args.epochs}'
    wandb.init(
        project="DAV-DNA_song_sgdd",
        job_type='FA',
        name=exp_name,
        config=args_dict
    )
    ...
    # 다른 모델을 쓰면 안되는 건가? 
    # E-step에서 쓰는 diffusion이 optimizing하고 있는 dav의 diffusion이어야 함, 정말 search하는 알고리즘만 바꿔야 함
    ...
    # load algorithm
    # algorithm = SGDD(model_sgdd, forward_op, alpha=args.alpha_sgdd, num_steps=2)
    algorithm = SGDD(model.ref_model, forward_op, alpha=args.alpha_sgdd, num_steps=args.num_steps_sgdd)
    ...
    for epoch in tqdm(range(args.epochs), desc="Training epochs: ", position=0):
        ...
        # )
        # with torch.no_grad():
        #     gen_samples, q_xs_history, x_history, q_x0_history, last_x_list = algorithm.inference(observation=torch.torch.zeros(args.batch_size), num_samples=args.batch_size)
        # with torch.no_grad():
        ...
        for inner_epoch in range(args.inner_epochs):
            ...
            for i in trange(division, desc=f"Policy Training Divisions (batch_size={len(gen_samples)//division})"):
                ...
                if args.kl_reg_coef > 0 :
                    ...
                    # print(f'total_loss without KL: {total_loss.item():.6f}')
                    total_loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--training_batch_size", type=int, default=50, help="batch division", required=False)
    parser.add_argument("--inner_epochs", type=int, default=2, help="inner epochs", required=False)
    ...
    parser.add_argument('--skip_steps', type=int, default=35, help='skip steps', required=False)
    parser.add_argument('--truncated_steps', type=int, default=0, help='truncated steps', required=False)
    parser.add_argument('--alpha_sgdd', type=float, default=30.0, help='alpha parameter for SGDD algorithm, guidance scaler', required=False)
    parser.add_argument('--batch_size_sgdd', type=int, default=50, help='batch size for sgdd sampling', required=False)
    parser.add_argument('--num_steps_sgdd', type=int, default=50, help='number of steps for sgdd sampling', required=False)
    parser.add_argument('--division', type=int, default=10, help='division for m-step training', required=False)
    # parser.add
